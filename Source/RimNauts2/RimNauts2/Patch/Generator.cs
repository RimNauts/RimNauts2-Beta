...
namespace RimNauts2.Patch {
    ...
    static class Generator_SendLetter {
        ...
        public static void Postfix(Universum.Defs.ObjectGeneration objectGenerationStep, List<string> celestialDefNames, List<Universum.World.ObjectHolder> objectHolders) {
            if (objectGenerationStep.defName == "RimNauts2_ObjectGeneration_AsteroidOre" && Settings.Container.get_asteroid_ore_verbose && Universum.Game.MainLoop.instance.GetTotal(Universum.Loader.Defs.CelestialObjects["RimNauts2_CelestialObject_Satellite_Relay"]) > 0) {
